...
function loadQuestion() {
    ...
    // TTS Setup
    ttsBtn.onclick = () => speak(getSpeakableText(question));
}

function getSpeakableText(q) {
    if (currentMode === 'complete-phrase') return q.text.replace('___', 'blank');
    if (currentMode === 'image-match') return "What is this? " + q.correct_answer;
...
function renderSyllableBuilder(q) {
    ...
    currentBuild.style.fontSize = '2em';
    currentBuild.style.marginBottom = '20px';
    ...
    syllables.forEach(syl => {
        ...
        btn.onclick = () => {
            ...
            if (builtSyllables.length === q.syllables.length) {
                const result = builtSyllables.join('');
                checkAnswer(result, q.correct_answer, currentBuild);
            }
        };
        interactionArea.appendChild(btn);
    });
...
// Function to save a new question (used by admin page)
